...
from auxgeo.dataset.skymap import SkymapDatasetEval, SkymapDatasetTrain
from auxgeo.transforms import get_transforms_train, get_transforms_val
from auxgeo.utils import setup_system, Logger
from auxgeo.trainer import train
from auxgeo.evaluate.vigor_and_skymap import evaluate, calc_sim
from auxgeo.loss import InfoNCE
...
class Configuration:
    ...
    seed = 1
    epochs: int = 40
    batch_size: int = 12  # keep in mind real_batch_size = 2 * batch_size
    verbose: bool = True
    ...
    gps_dict_path: str = r"/media/xiapanwang/主数据盘/xiapanwang/Codes/python/New_Geolocalization/0_Datasets/SkyMAP/gps_dict_same.pkl"  # gps_dict_cross.pkl | gps_dict_same.pkl
    ...
    # Dataset
    data_folder = r"/media/xiapanwang/主数据盘/xiapanwang/Codes/python/New_Geolocalization/0_Datasets/SkyMAP"
    ...
    model_path: str = "./checkpoints/skymap_same"
...
if __name__ == '__main__':
    ...
    shutil.copyfile("./auxgeo/model_modified.py", "{}/model.py".format(model_path))
    ...
    # print("\nModel: {ConvNeXt-base Timm Version.}\n")
    ...
    # Train
    train_dataset = SkymapDatasetTrain(data_folder=config.data_folder,
    ...
    reference_dataset_test = SkymapDatasetEval(data_folder=config.data_folder,
                                               split="test",
                                               img_type="reference",
                                               same_area=config.same_area,
    ...
    query_dataset_test = SkymapDatasetEval(data_folder=config.data_folder,
                                           split="test",
                                           img_type="query",
                                           same_area=config.same_area,
    ...
    if config.sim_sample:
        ...
        query_dataset_train = SkymapDatasetEval(data_folder=config.data_folder,
                                                split="train",
                                                img_type="query",
                                                same_area=config.same_area,
                                                transforms=ground_transforms_val,
        ...
        reference_dataset_train = SkymapDatasetEval(data_folder=config.data_folder,
                                                    split="train",
                                                    img_type="reference",
                                                    same_area=config.same_area,
                                                    transforms=sat_transforms_val,
