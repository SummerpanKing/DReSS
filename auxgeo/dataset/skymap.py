...
class SkymapDatasetTrain(Dataset):
    ...
    def __init__(self,
        ...
        if same_area:
            self.cities = ['Chicago', 'Johannesburg', 'London', 'Rio', 'Sydney', 'Taipei', 'Tokyo', 'Vancouver']
            # self.cities = ['Chicago']

        else:
            self.cities = ['Chicago', 'London', 'Sydney', 'Tokyo']
    ...
    def __getitem__(self, index):
        ...
        row_offset_resized = np.round(
            row_offset / 224 * reference_img.shape[1])  # -- 224 is the origin size of reference in SkyMAP
        col_offset_resized = np.round(col_offset / 224 * reference_img.shape[2])

        query_x, query_y = row_offset_resized, col_offset_resized

        if is_flip:
            query_x = reference_img.shape[2] - query_x
        ...
        if np.random.random() < self.prob_rotate:
            ...
            query_y, query_x = self.rotate_point_around_center(query_y, query_x,
        ...
        return query_img, query_bev, reference_img, label, [query_x, query_y]
...
class SkymapDatasetEval(Dataset):
    ...
    def __init__(self,
        ...
        if same_area:
            ...
            if split == "train":
                ...
                self.cities = ['Johannesburg', 'Rio', 'Taipei', 'Vancouver']
    ...
    def __getitem__(self, index):
        ...
        if len(img_path.split("/")[-1][:-4].split(",")) == 6:  # query
            _, _, lat, lon, _, _ = img_path.split("/")[-1][:-4].split(",")

        else:  # reference
            _, _, lon, lat = img_path.split("/")[-1][:-4].split("_")
